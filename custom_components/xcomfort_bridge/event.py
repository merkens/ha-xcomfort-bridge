...
class XComfortEventEntity(EventEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the actuator names this rocker is paired to."""
        device = self._client.get_device(self._device_id)
        if device is None:
            return {}
        control_ids = device.get("controlId", [])
        if not control_ids:
            return {}
        return {"controls": [self._client.get_device_name(cid) for cid in control_ids]}
