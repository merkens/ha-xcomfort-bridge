...
class ComponentSensorSpec:
    ...
    name_getter: Callable[[XComfortBridgeClient, int], str] | None = None
...
COMPONENT_SENSOR_SPECS: tuple[ComponentSensorSpec, ...] = (
    ...
    ComponentSensorSpec(
        ...
        use_comp_id=True,
        value_getter=lambda client, device_id: client.get_component_sum_request(device_id),
        device_ids_getter=lambda client: client.get_sum_request_device_ids(),
    ),
    ComponentSensorSpec(
        key="valve",
        name="Valve",
        ...
        entity_category=EntityCategory.DIAGNOSTIC,
        subscribe_mode="device",
        unique_id_prefix="device",
        value_getter=lambda client, device_id: client.get_heater_channel_valve(device_id),
        device_ids_getter=lambda client: client.get_heater_channel_device_ids(),
        name_getter=lambda client, device_id: (
            client.get_device(device_id) or {}
        ).get("name") or f"Channel {device_id}",
    ),
    ComponentSensorSpec(
        key="battery",
        name="Battery",
...
class XComfortComponentSensorEntity(SensorEntity):
    ...
    def __init__(
        ...
        entity_name = (
            spec.name_getter(client, device_id)
            if spec.name_getter is not None
            else spec.name
        )
